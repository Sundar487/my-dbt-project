...
supplier_wise_booking_total as (
    ...
    SELECT
        ...
        COALESCE(promo_discount::numeric * converted_exchange_rate::numeric, 0), 2) 
        AS FLIGHT_NET_TOTAL
     
    FROM final f
    inner join stg_booking_master bm 
    on bm.ff_booking_master_id = f.swbt_booking_master_id
    where ff_booking_type = 1
)

select * from supplier_wise_booking_total
-- where swbt_booking_master_id in ('40902', '41311', '43432')
-- where swbt_booking_master_id in ('43432', '43278')
